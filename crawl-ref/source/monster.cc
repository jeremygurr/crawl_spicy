                                 ...
                                 bool /*rescale*/) const
{
    ...
    switch (crawl_state.difficulty)
    {
        case DIFFICULTY_EASY:
            delay = div_rand_round(delay * 130, 100);
            break;
        case DIFFICULTY_STANDARD:
            break;
        case DIFFICULTY_CHALLENGE:
            delay = div_rand_round(delay * 85, 100);
            break;
        case DIFFICULTY_NIGHTMARE:
            delay = div_rand_round(delay * 70, 100);
            break;
        default:
...
bool monster::heal(int amount)
{
    ...
    {
        mprf("%s is healed (%d + %d -> %d)", name(DESC_PLAIN).c_str(), hit_points-amount, amount, hit_points);
    }
                   ...
                   string /*aux*/, bool cleanup_dead, bool attacker_effects)
{
    ...
    if (alive())
    {
        ...
        if (Options.debug_monster_hp)
        {
            mprf("%s is damaged (%d - %d -> %d)", name(DESC_PLAIN).c_str(), hit_points+amount, amount, hit_points);
        }
